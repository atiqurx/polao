...
function formatInline(raw: string) {
  ...
  t = t.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, `<a href="$2" target="_blank" rel="noopener noreferrer" class="text-blue-600 underline hover:text-blue-800">$1</a>`);
  t = t.replace(/`([^`]+)`/g, `<code class="rounded bg-neutral-100 px-1 py-0.5 font-mono text-[0.85em]">$1</code>`);
  t = t.replace(/(\*\*|__)(.+?)\1/g, `<strong class="font-semibold">$2</strong>`);
    ...
    return (
      <ul className="ml-5 list-disc space-y-1">
        {lines.map((l, i) => {
          ...
          return <li key={i} dangerouslySetInnerHTML={{ __html: formatInline(cleaned) }} />;
        })}
      </ul>
    );
  }
...
export default function ChatbotSidebar({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      text:
        "Hi! I'm your Polao assistant. I can analyze bias, compare sources, and summarize events. Choose a suggestion below or ask anything.",
      isUser: false,
      timestamp: new Date(),
    },
  ]);
    ...
    (async () => {
      try {
        ...
        const res = await fetch("/api/chatbot/suggest", {
          ...
          body: JSON.stringify({ titles: articleTitles?.slice(0, 25) ?? [], count: suggestionCount }),
        });
        const json = await res.json();
        if (!cancelled && Array.isArray(json?.suggestions) && json.suggestions.length) {
          setSuggestionsState(json.suggestions.slice(0, suggestionCount));
        } else if (!cancelled) {
          setSuggestionsState(suggestions.slice(0, suggestionCount));
        }
      } catch {
        if (!cancelled) setSuggestionsState(suggestions.slice(0, suggestionCount));
      } finally {
  ...
  return (
    <div className="sticky top-4">
      <div className={`flex ${heightClass} w-full flex-col overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-200/60`}>
        {/* header */}
        <div className="flex items-center justify-between px-3 py-2">
          <div className="min-w-0">
            <h3 className="truncate text-sm font-semibold text-gray-900">Polao Assistant</h3>
            <p className="text-[11px] text-gray-500">Bias & trends • Quick answers</p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="flex-1 space-y-2 overflow-y-auto bg-gray-50/60 p-3">
              ...
              <div
                className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm shadow-sm ${
                  message.isUser
                    ...
                    : "rounded-bl-sm bg-white text-gray-900 ring-1 ring-gray-100"
                }`}
              >
          ...
          {showSuggestions && (
            <div className="flex justify-start">
              <div className="rounded-2xl bg-white px-3 py-2 text-sm text-gray-900 ring-1 ring-gray-100">
                <div className="mb-1 text-[11px] uppercase tracking-wide text-gray-500">Suggested</div>
                ...
                {suggestionsLoading && !suggestionsState.length ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(suggestionsState.length ? suggestionsState : FALLBACK_SUGGESTIONS).map((s, i) => (
                      <button
                        key={i}
                        type="button"
                        onClick={() => handleSuggestionClick(s)}
                        className="rounded-full bg-gray-50 px-3 py-1 text-xs text-gray-700 ring-1 ring-gray-200 hover:bg-white hover:shadow-sm"
                      >
                        {s}
                      </button>
                    ))}
